...
async function handleConversionMode(updateUIFunction) {
    ...
    if (newMode === 'MOORE_TO_MEALY') {
        ...
        if (validationResult.type === 'error' && validationResult.message.includes('Moore Rule')) {
            setValidationMessage('Cannot convert: the current Moore machine is invalid. Details: ' + validationResult.message, 'error');
            modeSelect.value = MACHINE.type; // Revert selection
            return;
        }
        ...
        try {
            ...
            // Use the animated conversion wrapper
            ...
            // Final state check (should be MEALY after animation completes)
            if (MACHINE.type !== 'MEALY') {
                 MACHINE.type = 'MEALY'; // Ensure state is correct
            }
            modeSelect.value = 'MEALY';

        } catch (err) {
            customAlert('Conversion Failed', err.message);
            modeSelect.value = MACHINE.type; // Revert on failure
        } finally {
...
export function initializeUI() {
    ...
    // Note: initializeLibrary() is called via unified_main.js
    initializeLibrary();
    renderAll();
    updateUndoRedoButtons();
    if(checkAnswerBtn) checkAnswerBtn.style.display = 'none';
