...
export async function showStep(idx) {
    // We intentionally don't include the final step in the visual animation index, 
    // as it's handled by addLogMessage directly in runSimulation.
    ...
    if (step.end) {
        ...
        // Note: The final log message is now handled by runSimulation for clean logging.
        document.querySelector(`.state-circle[data-id="${step.current}"]`)?.classList.add('state-animating');
        return;
    }
