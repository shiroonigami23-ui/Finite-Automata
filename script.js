document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        // FIXED: Enforce exactly 1 initial state rule for ALL automaton types
        if (!MACHINE || !Array.isArray(MACHINE.states)) return;

        const initialStates = MACHINE.states.filter(s => s.initial);

        if (initialStates.length > 1) {
            // Keep first initial state, remove others
            initialStates.slice(1).forEach(s => s.initial = false);
        }

        // Ensure at least one state is initial if any states exist
    ...
    (MACHINE.transitions || []).forEach(t => {
        ...
        // IMPROVEMENT: Add a white "halo" behind the text for readability
    ...
    // The rest of the function (rendering states) remains the same.
    ...
    return Array.from(stateSet);
      ...
      // FIXED version of your convertNfaToDfa function
